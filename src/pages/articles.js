  ...
  return (
    ...
    >
      ...
      <FramerImage
        ...
        src={img}
        alt={title}
        className="w-96 z-10 hidden h-auto absolute rounded-lg md:!hidden"
        style={{ x: x, y: y }}
        initial={{ opacity: 0 }}
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li className="col-span-1 relative w-full p-4 bg-light border border-solid border-dark rounded-2xl dark:border-light dark:bg-dark">
      ...
      <p className="text-sm mb-2 font-medium dark:text-light">{summary}</p>
      <span className="text-primary font-bold dark:text-teal-400">{time}</span>
    </li>
  );
};
...
const Article = ({ img, title, date, link }) => {
  return (
    ...
    >
      <MovingImg title={title} link={link} img={img} target="_blank" />
      <span className="text-primary font-semibold dark:text-teal-400 sm:self-start sm:pl-0 xs:text-sm">
        {date}
      </span>
    </motion.li>
  );
...
const articles = () => {
  return (
    <>
      <Head>
        <title>Articles | Tedi Julianto</title>
